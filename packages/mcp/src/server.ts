...
export function registerTools(server: McpServer, opts: RegisterToolsOptions): void {
  ...
  server.registerTool(
    'get_timeline_types',
    {
      ...
      annotations: READ,
      description:
        'List the EDITOR timeline clip types (video, image, text, solid, audio, graphic) with their editable props, plus the track types (media, audio, text) and what each holds. Each clip type also carries a copy-pasteable `example` item skeleton, and the catalog carries a `creation` section documenting the CREATE ops (add_item, insert_track, insert_prebuilt_track) - so this one call tells you both what you can create (and the exact item shape to pass) and what you can set. Read this before building any clip with update_timeline. Requires the editor:read scope.',
      inputSchema: {},
    },
    async (_args, extra) => {
      try {
