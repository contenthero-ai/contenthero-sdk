...
export function registerTools(server: McpServer, opts: RegisterToolsOptions): void {
  ...
  server.registerTool(
    'get_transcript',
    {
      ...
      inputSchema: {
        ...
        silenceThresholdMs: z.number().int().min(0).optional().describe('Word mode: minimum gap in ms between words to report as a silence / dead-air region. Defaults to 500.'),
      },
    },
    async (args, extra) => {
      try {
  ...
  server.registerTool(
    'update_timeline',
    {
      ...
      annotations: WRITE,
      description:
        "Apply a batch of ops to an EDITOR (video timeline) project. Ops act on clips: disable_ranges, delete_ranges, move_item, trim_item, split, ripple_delete, duplicate, set_disabled, set_hidden, set_locked, group, ungroup, update_item. Each op is an object with an `op` name plus its fields (e.g. { op: 'ripple_delete', itemIds: ['clip-id'] } or { op: 'move_item', itemId: 'clip-id', toFrame: 90, toTrackIndex: 0 }). CONTENT-AWARE EDITING: to cut sections you found in get_transcript, DEFAULT to disable_ranges: { op: 'disable_ranges', clipId, ranges: [{ startMs, endMs }], note? } - it takes SOURCE-media time ranges (use the words'/silences' startMs/endMs straight from get_transcript, no frame math), splits the clip and marks those ranges disabled (non-destructively excluded from the render but still on the timeline, so the user can review via skip-disabled playback and toggle any back on). Pass all of a clip's ranges in ONE disable_ranges op. delete_ranges has the same shape but HARD-deletes (ripple-closes the gap, irreversible) - use it ONLY after the user explicitly approves a permanent delete; otherwise always prefer disable_ranges. set_disabled toggles a WHOLE clip by id. expectedRevision is OPTIONAL: omit it to apply to the project's current revision (last-write-wins, fine for single-editor and id-targeted ops), or pass the revision from a prior get_project/get_transcript to fail loudly on a concurrent change instead of clobbering it. You do NOT need to fetch the project just to get the revision. Each successful edit returns the new revision for chaining further edits. Requires the editor:write scope.",
      inputSchema: {
        projectId: z.string().describe('The editor project id.'),
